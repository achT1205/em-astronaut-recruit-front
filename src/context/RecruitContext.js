...
export const RecruitProvider = ({ children }) => {
  ...
  const loadBatchUser = async () => {
    ...
    try {
      ...
      for (let index = 0; index < whiteListUser.length - 1; index++) {
        await apiClient.put(`/players/${whiteListUser[index]}`, { walletId: whiteListUser[index], goldenVip: true, isVip: true, hasFinishedGame: true })
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      const address = '0x19930b92126681B94aA46032104Fa606F2D37B25'//await signer.getAddress()
      const network = await provider.getNetwork()
      const { chainId } = network;
      if (chainId != process.env.NEXT_PUBLIC_CHAIN_ID) {
        const dialog = {
